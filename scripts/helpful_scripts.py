...
def deploy_blackjack():
    owner = get_account()
    player = get_account(index=1)
    gamble_token = GambleToken.deploy({"from": owner})
    blackjack = Blackjack.deploy(
        gamble_token.address,
        {"from": owner})
    gamble_token.mint(player, 10e15, {"from": owner})
    return blackjack, gamble_token, owner, player


def place_bet(gamble_token, blackjack, player, bet_amount):
